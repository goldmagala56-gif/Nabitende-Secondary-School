...
export default function ParentGrades() {
  ...
  try {
    ...
    const res = await api.get('/grades', { params: { student_id: child.id } })
  ...
  return (
    <PageShell
      ...
      subtitle={student ? `${student.full_name} · ${student.class_name} · Term 2, 2026` : 'Term 2, 2026'}
      actions={
        <button className="flex items-center gap-2 px-4 py-2.5 rounded-xl
          border-2 border-[var(--color-border)] text-[13px] font-semibold
    ...
    >
      {loading ? (
        ...
        <>
          ...
          <div className={`flex items-center justify-between p-5
            ...
            <div className="text-right">
              <div className="text-[11px] text-[var(--color-text-muted)]
                uppercase tracking-widest mb-1">Aggregate</div>
          ...
          {/* Report card table */}
          <SectionCard title="Subject performance" noPadding>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[var(--color-border)]
                    bg-[var(--color-bg)]">
                    {['Subject', 'Bot 1', 'Bot 2', 'Average', 'Grade', 'Pts'].map(h => (
                    ...
                    return (
                      <tr key={r.subject}
                        ...
                        <td className="px-4 py-3 text-[13px]
                          text-[var(--color-text-muted)]">{r.bot1 ?? '—'}</td>
                        <td className="px-4 py-3 text-[13px]
                          text-[var(--color-text-muted)]">{r.bot2 ?? '—'}</td>
                        <td className="px-4 py-3 text-[13px] font-semibold
                          text-[var(--color-text)]">{r.average ?? '—'}</td>

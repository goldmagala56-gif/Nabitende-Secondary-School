...
import {
  Send, Megaphone, MessageSquare,
  ChevronDown, Users, ChevronLeft
} from 'lucide-react'
import api from '../../api'
...
export default function AdminMessages() {
  ...
  const [bcastRole,      setBcastRole]      = useState('teacher')
  ...
  useEffect(() => { fetchMessages(); fetchUsers() }, [])
  useEffect(() => { if (selected) fetchThread(selected) }, [selected])
  ...
  async function handleBroadcast(e) {
    ...
    try {
      ...
      setBcastSuccess(res.data.message)
      setBcastBody('')
      setBcastSubject('')
    } catch (err) {
  ...
  return (
    <PageShell
      ...
      subtitle={selected ? '' : `${unreadCount} unread message${unreadCount !== 1 ? 's' : ''}`}
    >
          ...
          {tab === 'broadcast' && (
            <SectionCard title="Broadcast message">
              <div className="flex items-start gap-3 p-4 rounded-xl
                bg-amber-50 border border-amber-200 mb-5">
                <Megaphone size={16} className="text-amber-600
                  flex-shrink-0 mt-0.5" />
                <p className="text-[13px] text-amber-700">
                  Sends your message to <strong>all users</strong> of
                  the selected role at once. Use for announcements,
                  deadlines or urgent notices.
                </p>
              </div>
              ...
              <form onSubmit={handleBroadcast} className="space-y-4">
                    ...
                    {['teacher', 'parent', 'student'].map(role => {
                      ...
                      return (
                        <button key={role} type="button"
                          onClick={() => setBcastRole(role)}
                          className={`flex items-center gap-2 px-4 py-2.5
                            rounded-xl border-2 text-[13px] font-semibold
                            transition-colors capitalize ${
                            bcastRole === role
                              ? `${rc.bg} ${rc.text} border-current`
                              : 'border-[var(--color-border)] text-[var(--color-text-muted)]'
                            }`}>
                          <Users size={14} />
                          All {role}s
                ...
                <div className="flex justify-end">
                  <button type="submit" disabled={broadcasting}
                    className="flex items-center gap-2 px-6 py-2.5
                      rounded-xl bg-[#ea580c] hover:bg-[#c2410c]
                      text-white text-[14px] font-semibold
                      transition-colors disabled:opacity-60">
                    <Megaphone size={16} />
                    {broadcasting
                      ? 'Broadcasting...'
                      : `Broadcast to all ${bcastRole}s`
                    }
                  </button>
                </div>
              </form>
